...
**Phase 4 contracts (#241).** Watcher becomes the `content.fetch` issuer and `content.blobs` consumer at the Phase-4 cutover (in progress; design: `docs/plans/2026-08-06-phase-4-content-fetch-producer-design.md`). Step 1 is merged **inert**: the `fetch_commands` table (outbox + pending map + inbox keyed `command_id`, `src/core/fetch_commands.py`) and an issue path inside `check_watched_item` behind `WATCHER_FETCH_MODE` (default `local` = today's inline fetch; `bus` issues commands with the pinned watcher UA, gated on one open command per item, persist-before-publish with an every-minute `publish_pending_fetch_commands` sweep). Do not set `WATCHER_FETCH_MODE=bus` in prod before the design's step 2 (consumer + apply + reaper) lands — commands would be fetched by Replicator and their facts never applied. The normative contracts live in the Replicator repo — **link, don't copy**: [`content-fetch-issuer-contract.md`](https://github.com/CannObserv/replicator/blob/main/docs/contracts/content-fetch-issuer-contract.md) (the seven MUSTs: per-occasion `command_id`, persist-before-publish, correlate on `command_id` only, idempotent upsert, no fingerprint dedupe, handle `fetch_failed` + keep a reaper, copy blob bytes before expiry — and note `blob_uri` is a host-local `file://`, VM-local by contract) and [`replicator-boundaries.md`](https://github.com/CannObserv/replicator/blob/main/docs/contracts/replicator-boundaries.md) (what belongs on which side of the fetch boundary). Cutover ordering blocker: #245 (politeness must not lapse when the fetch path becomes a publish path).
