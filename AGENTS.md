...
**Watched Item detail** (`#174`, updated `#185`, `#190`, `#191`, `#199`, `#202`): a single **Details** panel consolidates the Status pause/resume toggle (the sole status badge), the `effective_url` row (inline re-probe **Edit**), `domain_name`, the editable `name`/`description`/Interval/Content Type/Tags fields, and read-only `last_checked_at` (with the Check-now action), `last_changed_at`, and `health_status` (shown even when UNKNOWN) — all from local WatchedItem columns, no Archiver SDK calls. Interval shows the inherited system default when unset. Below it: a read-only `source_specs` panel, then the **Notification Templates** panel (item-scoped CRUD plus read-only **Global**/**Domain** inherited sections that fire at dispatch — parity with the Domain detail page; #199, unified in #200), then **Recent Activity**. `POST /watched-items/{id}/mark-reviewed` (stamps `last_reviewed_at`) remains API-only — the dashboard route exists but is intentionally unwired; no dashboard UI until a replacement is designed.
