...
src/core/diff/         — Dashboard/notification diff service: normalize → compute → render (models.py: frozen+slots DiffResult [unified_diff, has_changes, added, removed]; normalize.py: normalize_text [CRLF/CR→LF + per-line rstrip], normalize_html [#118: html5lib parse + lxml.html.tostring(pretty_print=True), strips comments, preserves whitespace inside `<pre>`/`<textarea>`/`<script>`/`<style>`, idempotent via two-pass fixed-point (fast-path skips pass 2 when input is already a fixed point)]; textual.py: compute_unified_diff(prev, curr, *, context=3) via stdlib difflib; uses identical fromfile/tofile label "content" so diff2html doesn't emit a RENAMED badge; structural.py: compute_html_tree_diff stub raising NotImplementedError, reserved for #115 Phase B.2 xmldiff integration)
