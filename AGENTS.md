...
src/api/deps.py          — FastAPI dependencies: `get_db_session` (async session injection), `get_probe_fn` (URL probe; override in tests), `require_api_key` (X-API-Key auth; validates X-API-Key header (403 absent, 401 invalid), updates last_used_at, returns user_id; guards entire /api/v1/ router)
