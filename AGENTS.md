...
src/core/notifications/  — Apprise-based notification dispatcher; WatchEvent + WatchEventType + EVENT_TITLES (events.py; WatchEvent has no .title/.body — rendered from templates by the dispatcher), dispatch_event(*, body, title) → DispatchResult (dispatcher.py; body + title required, no fallback); _ASSET (AppriseAsset) applies CO Watcher branding to all outbound notifications; captures Apprise WARNING logs into DispatchResult.reason on failure (ContextVar-isolated per asyncio task); content.py: resolve_options() + build_title(*, strict=False) + build_body(*, strict=False) + render_template() (Jinja2, falls back to raw string on error) + render_template_strict() (StrictUndefined — raises on syntax + undefined; for preview) + build_template_context() (includes event_label + change_summary derived fields); default_templates.py: DEFAULT_TITLE_TEMPLATES + DEFAULT_BODY_TEMPLATES (Jinja strings, keyed by event_type value — single source of truth for built-in defaults); preview_fixtures.py: MOCK_EVENT_FIXTURES (canned per-event metadata) + build_preview_event() (stateless WatchEvent for preview endpoint)
