...
src/workers/notify.py    — Notification dispatch: dispatch_event_notifications(session, event) — unions local WatchNotificationConfig rows with NotificationTemplate rows joined via WatchNcRef into DispatchCandidate list; source field is "local"|"template"
