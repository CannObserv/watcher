...
src/core/notifications/  — Apprise-based notification dispatcher; WatchEvent + WatchEventType + EVENT_TITLES (events.py), dispatch_event() → DispatchResult (dispatcher.py); _ASSET (AppriseAsset) applies CO Watcher branding to all outbound notifications; captures Apprise WARNING logs into DispatchResult.reason on failure (ContextVar-isolated per asyncio task); content.py: resolve_options() + build_body() + render_template() (Jinja2, falls back to raw string on error) + build_template_context()
