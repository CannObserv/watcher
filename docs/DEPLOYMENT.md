...
| `WATCHER_FETCH_MODE` | env | no | `local` (code default) fetches inline; `bus` issues `content.fetch` commands to Replicator (#241 Phase 4). **Prod is `bus` since 2026-08-06.** `local` remains the rollback path until step 5 removes it — see AGENTS.md § Phase 4 contracts |
