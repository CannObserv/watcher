...
| `WATCHER_PUBLIC_BASE_URL` | `/etc/watcher/.env` | no (prod: set it) | The dashboard's public base (`https://co-watcher.exe.xyz` after #296), from which notification links are built — the `app_url` template variable, the ITEM line and `change_url` (#296 D6). Unset → links are **omitted**, never a guessed host, and startup WARNs when the notifier is enabled. Malformed (no scheme, not http(s), a query or fragment) → **startup refuses**, since it is a typo that would otherwise drop every link silently. Not a credential, so it lives in the shared file; `scripts/dev_server.sh` still clears it so a dev worker's links cannot point into production |
