...
**The backup unit loads its own two files, and neither of the service's** (#296 D8/D9): `/etc/watcher/backup.env` (bucket and writer-key path) and `/etc/watcher/backup-notifier.env` (0400 root:root, the dead-man check-in). `watcher-backup.service` never loads `/etc/watcher/.env` — it holds no database credential and must not hold the bus URL — nor `notifier.env`. See [RECOVERY.md](RECOVERY.md).
...
| `WATCHER_BACKUP_NOTIFIER_BASE_URL` | `/etc/watcher/backup-notifier.env` | backup unit | Notifier base for the dead-man check-in (#296 D9). Configuration, not a constant — see RECOVERY.md for why a port typo still alarms rather than inverts |
| `WATCHER_BACKUP_MONITOR_ID` | `/etc/watcher/backup-notifier.env` | backup unit | The notifier monitor the backup checks in to on every run; all three `WATCHER_BACKUP_NOTIFIER_*`/`_MONITOR_ID` or none |
| `WATCHER_BACKUP_NOTIFIER_API_KEY` | `/etc/watcher/backup-notifier.env` | backup unit | `production`-marked check-in key. Distinct from `WATCHER_NOTIFIER_API_KEY`, which stays `watcher.service`'s alone (#278) |
