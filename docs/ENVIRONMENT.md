...
**The backup unit loads one env file of its own, and neither of the service's** (#296 D8/D9, #297): `/etc/watcher/backup.env` — the bucket, and the check-in's base URL and monitor id, no secret. Its two keys are not environment at all: `LoadCredential=` reads `/etc/watcher/co-watcher-backup.json` and `/etc/watcher/backup-notifier.key` (both `0400 root:root`, both required; the second is an extra `production` key on the watcher tenant — distinct from `WATCHER_NOTIFIER_API_KEY`, and waiting on notifier#62 to be mintable — and empty until the monitor exists) and hands the run private copies under `$CREDENTIALS_DIRECTORY`. `watcher-backup.service` never loads `/etc/watcher/.env` — it holds no database credential and must not hold the bus URL — nor `notifier.env`. See [RECOVERY.md](RECOVERY.md).
