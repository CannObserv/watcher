...
| `WATCHER_BACKUP_PREFIX` | `/etc/watcher/backup.env` | no | Object prefix the backup writes under; defaults to the hostname, so each host's dumps list as their own timeline. The restore never reads it — a restore runs on another host, so `--prefix` names the source |
