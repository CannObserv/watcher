...
### Shared index on `co-index`

The store and the embedder live on `co-index`, not here, and `includeLinked: true`
answers from every cohort repo. The client contract is two committed files plus a
per-host key, and five of its failure modes report themselves as green — all of it
in **[SOCRATICODE.md](SOCRATICODE.md)**, pinned by
[tests/deploy/test_socraticode_config.py](../tests/deploy/test_socraticode_config.py).
