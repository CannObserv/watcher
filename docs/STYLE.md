...
- **Modal dialogs** (#39): every `aria-modal="true"` element must carry `data-focus-trap` — `focus-trap.js` supplies the behavior the semantics promise: focus moves into the dialog on open (`[data-focus-trap-initial]` preferred and pre-selected, else the first rendered focusable), Tab/Shift-Tab contained across rendered elements only, Escape closes, focus restored to the trigger. Guarded by `tests/dashboard/test_focus_trap.py`, and `role="dialog"` must declare `aria-modal="true"` so no dialog escapes that guard. Dialogs HTMX swaps in arm automatically on `htmx:afterSwap`; dialogs with their own open/close wiring call `window.focusTrap.activate(dialog, {onEscape, restoreTo})` / `.deactivate(dialog)` (the mobile drawer) — pass `restoreTo` whenever the trigger is known, since the auto-captured `activeElement` can be `<body>` or an element HTMX later swaps away. Where closing is destructive, opt out of Escape with `data-focus-trap-escape="ignore"` — the one-time API-key modal does; key loss must never be a one-keystroke action. Open/close transitions, if added, must respect `prefers-reduced-motion` (the global reduced-motion rule above already covers CSS transitions).
