...
- **Architectural** — new projects, new subsystems, changes that
  ...
  design, written design doc, GitHub issue, then a worktree.
...
**Bounded:**
1. **Explore project context** — read AGENTS.md, check files and recent commits
2. **Ask clarifying questions** — one at a time, the ones that matter
3. **Present short design in chat** — approach, files touched, testing
4. **Get approval** — STOP and wait for an explicit yes; presenting the design and starting in the same breath is skipping the gate
5. **Implement** — the normal watcher workflow, which is TDD (red → green → refactor, per AGENTS.md); no plan document

**Architectural:**
1. **Explore project context** — read AGENTS.md, check files, docs, recent commits
...
8. **User reviews written design doc** — ask user to review the file before proceeding
9. **Open a GitHub issue** — link the design doc; report the issue number
10. **Set up a worktree** — invoke `using-git-worktrees` for any multi-step implementation
11. **Hand off** — proceed to implementation, or invoke `writing-plans` if a formal plan is wanted (optional)
...
digraph brainstorming {
    ...
    "Implement via TDD (no plan doc)" [shape=doublecircle];
    "Explore project context" [shape=box];
    "Ask clarifying questions" [shape=box];
    "Propose 2-3 approaches" [shape=box];
    "Present design sections" [shape=box];
    "User approves design?" [shape=diamond];
    "Write design doc (docs/plans/)" [shape=box];
    "Spec self-review\n(fix inline)" [shape=box];
    "User reviews design doc?" [shape=diamond];
    "Open GitHub issue" [shape=box];
    "Set up worktree; implement" [shape=doublecircle];
    ...
    "Human approves?" -> "Implement via TDD (no plan doc)" [label="bounded: yes"];
    ...
    "User approves design?" -> "Write design doc (docs/plans/)" [label="yes"];
    "Write design doc (docs/plans/)" -> "Spec self-review\n(fix inline)";
    "Spec self-review\n(fix inline)" -> "User reviews design doc?";
    "User reviews design doc?" -> "Write design doc (docs/plans/)" [label="changes requested"];
    "User reviews design doc?" -> "Open GitHub issue" [label="approved"];
    "Open GitHub issue" -> "Set up worktree; implement";
}
```

**Terminal states are path-bound.** Architectural: after the issue and
the worktree, implementation proceeds; `writing-plans` is available but
optional, and no other skill is invoked without asking. Bounded: after
approval, implementation proceeds directly through TDD; no plan
...
- Check out the current project state first (AGENTS.md, files, docs, recent commits)
- Before asking detailed questions, assess scope: if the request describes multiple independent subsystems, flag this immediately. Don't spend questions refining details of a project that needs to be decomposed first.
- If the project is too large for a single design doc, help the user decompose into sub-projects: what are the independent pieces, how do they relate, what order should they be built? Then brainstorm the first sub-project through the normal design flow.
...
- Write the validated design to `docs/plans/YYYY-MM-DD-<topic>-design.md`
  - (User preferences for the location override this default)
...
**User Review Gate:**
After the spec review loop passes, ask the user to review the written design doc before proceeding:

> "Design doc written and committed to `<path>`. Please review it and let me know if you want to make any changes before we start implementation."
...
- Proceed to implementation under TDD — AGENTS.md requires a failing test first
- For multi-step work, `writing-plans` may be invoked to create a task-by-task plan. This is **optional** in watcher, not the mandatory next step it is upstream.
