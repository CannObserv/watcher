...
## Process Log — Session 2026-05-08

**Backlog cleared:** #143, #144, #150 (Batch A, 3 parallel agents), #137 (Batch B, 1 agent). #140 explicitly deferred during interview. Tracking issue #151. Spawned #154 as a CR follow-up. Both batches landed on `main` and deployed to production.

**Patterns confirmed worth recording for future invocations:**

1. **Rule 3 is conditional, not universal — `isolation: "worktree"` does NOT auto-merge when the worker explicitly creates a named feature branch.** This session's worker briefings told each agent to commit to a specific feature branch (e.g. `feature/batch-a-143-consumer-hardening`). The agents followed the instruction inside their `.claude/worktrees/agent-XXX/` worktree, and their commits stayed on those feature branches. The orchestrator's checked-out branch (`batch/a-phase2c-followups`) saw zero auto-merged commits — only the design-doc commit was there until I manually `git merge --no-ff feature/batch-a-NNN` for each.

   **Practical adjustment:**
   - The "check out batch/<X> before spawning agents" pattern is still useful — but only as a base point.
   - If the worker prompt names a feature branch, plan to **manually merge each worker branch into the batch branch** after they signal completion. Use `git merge --no-ff feature/batch-<x>-<issue> -m "..."` per worker.
   - If you want the auto-merge-to-current-branch behaviour, do NOT instruct the worker to create a named feature branch — let them work on the temporary worktree branch the harness creates. Trade-off: per-worker git history is harder to inspect.
   - Verifiable end-state: after all workers signal, run `git log main..HEAD` on the batch branch. If commits are missing, look for them on `feature/batch-<x>-*` branches and merge.

2. **Plan for 2–5 CR rounds per batch, not one.** Both batches in this session went through multiple `/reviewing-code-claude` passes before findings stabilized at observation-only. The diminishing-returns curve was consistent:
   - **Batch A**: round 1 = 12 findings → round 2 = 5 → round 3 = 1 real + 2 minor. Three rounds total.
   - **Batch B**: round 1 (post-merge) = 6 findings → round 2 = 5. Two rounds total.

   The first round catches the substantive issues (correctness, leaky abstractions, broken docstrings); subsequent rounds catch the corrections-to-corrections (e.g. round 2 of Batch B found a regression introduced by round 1's `channel_hint` simplification — empty strings now passing through where they used to be normalized).

   **Practical adjustment:**
   - Don't ship after the first CR round. Always run a second to surface corrections-to-corrections.
   - Stop when a round's findings are mostly 💭 observations with at most one 🟡 nit. That's the convergence signal.
   - Time-budget: a round is 5–15 minutes of orchestrator work plus user directive turnaround. Three rounds per batch = ~30–45 minutes of CR cadence on top of the worker's implementation time.
   - Single-agent batches (Batch B here) tend to need fewer rounds than multi-agent batches because there's no inter-agent inconsistency to reconcile, but they're still not 1-and-done.

**Other observations (not yet promoted to rules):**

- **Worktree cleanup is not automatic.** Several `worktree-agent-XXX` directories stayed locked in `git worktree list` at the end of the session even after the agents completed. Doesn't break anything, but is filesystem clutter. `git worktree prune` after each batch lands would tidy this.
- **Shipping isn't part of this skill.** The orchestration arc ends at "design doc + tracking issue". Production deploy (alembic migration, systemd restart, smoke checks) lives in `shipping-work-claude`. When `#137`-style work touches DB schema or systemd lifecycle, plan to invoke `shipping-work-claude` after the merge — and the user will need to authorize destructive ops on the prod DB explicitly.
- **The interview's deployment-context question maps directly to risk tolerance for issue 137-style strips.** "Pre-production" was the answer this session, which made the irreversible-in-practice column drop acceptable. In "active production" we'd have either deferred the destructive part or shipped it behind a `feature_disabled` flag instead of a column drop.
