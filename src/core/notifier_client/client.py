...
``NOTIFIER_ENABLED=1`` is therefore additionally required to build a client at
...
NOTIFIER_BASE_URL_ENV = "NOTIFIER_BASE_URL"
NOTIFIER_API_KEY_ENV = "NOTIFIER_API_KEY"

#: Unit-only opt-in gating every notifier client this process can build (#277).
NOTIFIER_ENABLED_ENV = "NOTIFIER_ENABLED"
...
def notifier_enabled(environ: Mapping[str, str] | None = None) -> bool:
    ...
    if environ is None:
        environ = os.environ
    return environ.get(NOTIFIER_ENABLED_ENV) == "1"
...
def assert_environment_notifier_allowed(environ: Mapping[str, str]) -> None:
    ...
    ``Environment=NOTIFIER_ENABLED=1`` from the unit and Watcher stops notifying
    ...
    """
    if not environ.get(NOTIFIER_BASE_URL_ENV):
        return
    if notifier_enabled(environ):
        return
    raise NotifierNotEnabled(
        f"refusing to start: {NOTIFIER_BASE_URL_ENV} is set but {NOTIFIER_ENABLED_ENV} "
        "is not 1, so this process holds a notifier address it is not authorised "
        "to use.\n"
        f"  Only deploy/watcher.service may notify the production tenant; it sets "
        f"{NOTIFIER_ENABLED_ENV}=1. If this IS the service, that line is missing from "
        "the installed unit.\n"
        "  For a dev server use: bash scripts/dev_server.sh (set "
        "WATCHER_DEV_NOTIFIER_BASE_URL for a scratch notifier).\n"
        f"  For anything else, unset {NOTIFIER_BASE_URL_ENV}. (See #277.)"
    )
...
def get_notifier_client() -> NotifierClient:
    ...
    Raises RuntimeError if NOTIFIER_BASE_URL or NOTIFIER_API_KEY are unset, and
    :class:`NotifierNotEnabled` if they are set without ``NOTIFIER_ENABLED=1``.
    ...
    """
    base_url = os.environ.get(NOTIFIER_BASE_URL_ENV)
    if not base_url:
        raise RuntimeError(f"{NOTIFIER_BASE_URL_ENV} environment variable is required")
    api_key = os.environ.get(NOTIFIER_API_KEY_ENV)
    if not api_key:
        raise RuntimeError(f"{NOTIFIER_API_KEY_ENV} environment variable is required")
