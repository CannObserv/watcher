...
@pytest.mark.asyncio
async def test_select_unpublished_returns_only_unpublished(
    db_session, make_watch, make_snapshot, make_change
):
    watch = await make_watch()
    s1 = await make_snapshot(watch=watch, **_snapshot_kwargs())
...
@pytest.mark.asyncio
async def test_select_unpublished_orders_by_detected_at(
    db_session, make_watch, make_snapshot, make_change
):
...
@pytest.mark.asyncio
async def test_mark_published_sets_fields(db_session, make_watch, make_snapshot, make_change):
    watch = await make_watch()
    s1 = await make_snapshot(watch=watch, **_snapshot_kwargs())
    s2 = await make_snapshot(watch=watch, **_snapshot_kwargs("hash2"))
...
@pytest.mark.asyncio
async def test_select_unpublished_respects_limit(
    db_session, make_watch, make_snapshot, make_change
):
    watch = await make_watch()
    s1 = await make_snapshot(watch=watch, **_snapshot_kwargs())
