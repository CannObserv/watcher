...
class TestCsvExtractor:
    ...
    async def test_small_csv_single_chunk(self):
        csv_bytes = _make_csv(rows=50)
        result = await self.extractor.extract(csv_bytes, config={"content_type": "csv"})
        assert len(result.chunks) == 1
        assert result.chunks[0].chunk_type == "row_range"
        assert result.chunks[0].label == "Rows 1-50"

    async def test_large_csv_multiple_chunks(self):
    ...
    async def test_chunk_row_size_configurable(self):
    ...
    async def test_empty_csv_returns_empty(self):
        result = await self.extractor.extract(b"col1,col2\n", config={"content_type": "csv"})
        assert len(result.chunks) == 0

    async def test_csv_text_includes_header(self):
...
class TestXlsxExtractor:
    ...
    async def test_xlsx_basic_extraction(self):
        xlsx_bytes = _make_xlsx(rows=5)
        result = await self.extractor.extract(xlsx_bytes, config={"content_type": "xlsx"})
        assert len(result.chunks) == 1
        assert result.chunks[0].chunk_type == "row_range"
        assert result.chunks[0].label == "Rows 1-5"

    async def test_xlsx_text_includes_header(self):
    ...
    async def test_xlsx_multiple_chunks(self):
    ...
    async def test_xlsx_empty_returns_empty(self):
        wb = Workbook()
        ws = wb.active
        ws.append(["col1", "col2"])  # header only
        buf = io.BytesIO()
        wb.save(buf)
        result = await self.extractor.extract(buf.getvalue(), config={"content_type": "xlsx"})
        assert len(result.chunks) == 0
