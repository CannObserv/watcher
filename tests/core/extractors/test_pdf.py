...
class TestPdfExtractor:
    ...
    async def test_extracts_one_chunk_per_page(self):
        pdf_bytes = _make_pdf_bytes(3)
        result = await self.extractor.extract(pdf_bytes)
    ...
    async def test_empty_pages_excluded(self):
        pdf_bytes = _make_pdf_bytes(2)
        result = await self.extractor.extract(pdf_bytes, config={"skip_empty_pages": True})
        assert len(result.chunks) == 0

    async def test_chunk_type_is_page(self):
    ...
    async def test_invalid_pdf_raises(self):
        with pytest.raises(ValueError, match="Failed to parse PDF"):
            await self.extractor.extract(b"not a pdf")
