...
class TestWatchDetailPage:
    ...
    async def test_detail_page_has_status_toggle(self, client, db_session):
        watch = Watch(name="Toggle Test", url="https://example.com", content_type=ContentType.HTML)
    ...
    async def test_detail_page_shows_content_type_readonly(self, client, db_session):
        watch = Watch(name="CT Read", url="https://example.com", content_type=ContentType.PDF)
        db_session.add(watch)
        await db_session.flush()
        response = await client.get(f"/watches/{watch.id}")
        assert response.status_code == 200
    ...
    async def test_detail_page_shows_html_specific_fields(self, client, db_session):
        watch = Watch(name="HTML Fields", url="https://example.com", content_type=ContentType.HTML)
        db_session.add(watch)
        await db_session.flush()
        response = await client.get(f"/watches/{watch.id}")
    ...
    async def test_detail_page_shows_pdf_specific_fields(self, client, db_session):
        watch = Watch(name="PDF Fields", url="https://example.com", content_type=ContentType.PDF)
        db_session.add(watch)
        await db_session.flush()
        response = await client.get(f"/watches/{watch.id}")
        content = response.content.decode()
    ...
    async def test_detail_page_shows_file_specific_fields(self, client, db_session):
        watch = Watch(name="File Fields", url="https://example.com", content_type=ContentType.FILE)
        db_session.add(watch)
        await db_session.flush()
        response = await client.get(f"/watches/{watch.id}")
        content = response.content.decode()
    ...
    async def test_detail_page_has_danger_zone(self, client, db_session):
        watch = Watch(name="DZ Test", url="https://example.com", content_type=ContentType.HTML)
        db_session.add(watch)
        await db_session.flush()
        response = await client.get(f"/watches/{watch.id}")
        content = response.content.decode()
    ...
    async def test_detail_page_has_metadata_footer(self, client, db_session):
        watch = Watch(name="Meta Test", url="https://example.com", content_type=ContentType.HTML)
        db_session.add(watch)
        await db_session.flush()
        response = await client.get(f"/watches/{watch.id}")
        content = response.content.decode()
    ...
    async def test_detail_page_no_edit_button_in_header(self, client, db_session):
        ...
        watch = Watch(name="No Edit Btn", url="https://example.com", content_type=ContentType.HTML)
        db_session.add(watch)
        await db_session.flush()
        response = await client.get(f"/watches/{watch.id}")
        content = response.content.decode()
...
class TestWatchFieldPartialGet:
    ...
    async def test_field_partial_view_mode_default(self, client, db_session):
        watch = Watch(name="Field View", url="https://example.com", content_type=ContentType.HTML)
        db_session.add(watch)
        await db_session.flush()
    ...
    async def test_field_partial_edit_mode(self, client, db_session):
        watch = Watch(name="Field Edit", url="https://example.com", content_type=ContentType.HTML)
        db_session.add(watch)
        await db_session.flush()
        response = await client.get(
    ...
    async def test_field_partial_invalid_field_returns_400(self, client, db_session):
        watch = Watch(name="Bad Field", url="https://example.com", content_type=ContentType.HTML)
        db_session.add(watch)
        await db_session.flush()
    ...
    async def test_field_partial_non_htmx_redirects(self, client, db_session):
        watch = Watch(name="No HTMX", url="https://example.com", content_type=ContentType.HTML)
        db_session.add(watch)
        await db_session.flush()
        response = await client.get(
            f"/watches/{watch.id}/field/name",
...
class TestWatchFieldUpdate:
    ...
    async def test_update_name_returns_view_mode(self, client, db_session):
        watch = Watch(name="Old Name", url="https://example.com", content_type=ContentType.HTML)
        db_session.add(watch)
        await db_session.flush()
    ...
    async def test_update_interval(self, client, db_session):
        watch = Watch(name="Interval", url="https://example.com", content_type=ContentType.HTML)
        db_session.add(watch)
        await db_session.flush()
        response = await client.post(
    ...
    async def test_update_timeout_fetch_config(self, client, db_session):
        watch = Watch(name="Timeout", url="https://example.com", content_type=ContentType.HTML)
        db_session.add(watch)
        await db_session.flush()
        response = await client.post(
    ...
    async def test_update_ignore_patterns(self, client, db_session):
        watch = Watch(name="Patterns", url="https://example.com", content_type=ContentType.HTML)
        db_session.add(watch)
        await db_session.flush()
        response = await client.post(
    ...
    async def test_update_toggle_field(self, client, db_session):
        watch = Watch(name="Toggle", url="https://example.com", content_type=ContentType.HTML)
        db_session.add(watch)
        await db_session.flush()
        response = await client.post(
    ...
    async def test_update_invalid_field_returns_400(self, client, db_session):
        watch = Watch(name="Bad", url="https://example.com", content_type=ContentType.HTML)
        db_session.add(watch)
        await db_session.flush()
        response = await client.post(
    ...
    async def test_non_htmx_redirects(self, client, db_session):
        watch = Watch(name="Redirect", url="https://example.com", content_type=ContentType.HTML)
        db_session.add(watch)
        await db_session.flush()
        response = await client.post(
            f"/watches/{watch.id}/field/name",
...
class TestWatchStatusToggle:
    ...
    async def test_toggle_active_to_inactive(self, client, db_session):
        watch = Watch(
            name="Active", url="https://example.com", content_type=ContentType.HTML, is_active=True
        )
        db_session.add(watch)
        await db_session.flush()
        response = await client.post(
...
class TestWatchArchiveRestore:
    ...
    async def test_archive_sets_flags(self, client, db_session):
        watch = Watch(name="To Archive", url="https://example.com", content_type=ContentType.HTML)
        db_session.add(watch)
        await db_session.flush()
        response = await client.post(
    ...
    async def test_non_archived_detail_shows_archive_button(self, client, db_session):
        watch = Watch(name="Active DZ", url="https://example.com", content_type=ContentType.HTML)
        db_session.add(watch)
        await db_session.flush()
        response = await client.get(f"/watches/{watch.id}")
        content = response.content.decode()
